                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <select id="role" name="role" onchange="toggleTableSelect()"
                            ...
                            <option value="1" ${targetUser.isAdmin ? 'selected' : ''}>Administrator (管理者)</option>
                            <option value="2" ${targetUser.isKitchen ? 'selected' : ''}>Kitchen (キッチン)</option>
                            <option value="3" ${targetUser.isHall ? 'selected' : ''}>Hall (ホール)</option>
                            <option value="4" ${targetUser.isCashier ? 'selected' : ''}>Cashier (レジ)</option>
                            <option value="10" ${targetUser.isTableTerminal ? 'selected' : ''}>Table Terminal (テーブル端末)</option>
                        </select>
                    </div>

                    <!-- テーブル割り当て -->
                    <div id="table-select-container" class="space-y-4 ${targetUser.isTableTerminal ? '' : 'opacity-20 pointer-events-none transition-all duration-500'}">
